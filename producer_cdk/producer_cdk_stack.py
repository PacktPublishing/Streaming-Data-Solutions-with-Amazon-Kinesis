...
class ProducerCdkStack(core.Stack):
    ...
    def __init__(self, scope: core.Construct, construct_id: str, **kwargs) -> None:
        ...
        fhIAMRole.add_to_policy(
            iam.PolicyStatement(resources= [kda_output_bucket.bucket_arn, kda_output_bucket.bucket_arn+"/*"] , actions= ['s3:*'])
        )

        bufferingHints = fh.CfnDeliveryStream.BufferingHintsProperty(interval_in_seconds=60, size_in_m_bs=1)
        ...
        user_data = "#!/bin/bash\n"
        user_data += "echo KINESIS_STREAM=\"" + sourceStreamName.value_as_string +"\"\n | sudo tee -a /etc/profile"
        user_data += "echo export KINESIS_STREAM\n | sudo tee -a /etc/profile"
